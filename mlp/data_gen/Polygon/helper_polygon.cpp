...
void random_sub_scene_gen(Scene2D& scene, int pointCount, int cellXCount, int cellYCount, int subXCount, int subYCount, ofstream& out, ofstream& out2) {
    double cellX,cellY;
    int perGridPoint;
    cellX = (scene.maxX-scene.minX)/cellXCount;
    cellY = (scene.maxY-scene.minY)/cellYCount;
    ...
    int maxClassCount = pointCount*.6;
    vector<vector<int>> encodedScene = scene.encodeScene(cellXCount,cellYCount);
    for (int i=0;i<cellXCount/subXCount;i++) {
        for (int j=0;j<cellYCount/subYCount;j++) {
            for (int l=0;l<subXCount;l++) {
                for (int m=0;m<subYCount;m++) {
                    if (encodedScene[i*subXCount+l][j*subYCount+m]>0) {
                        out2<<1<<" ";
                    }
                    else {
                        out2<<0<<" ";
